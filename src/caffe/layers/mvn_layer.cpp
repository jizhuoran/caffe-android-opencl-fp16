...
void MVNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    // compute variance using var(X) = E((X-EX)^2)
    caffe_gpu_powx(bottom[0]->count(), top_data, float(2),
    ...
    // normalize variance
    caffe_gpu_powx(variance_.count(), variance_.gpu_data(), float(0.5),
          variance_.mutable_gpu_data());

    caffe_gpu_add_scalar(variance_.count(), eps_, variance_.mutable_gpu_data());

    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, dim, 1, float(1.),
          variance_.gpu_data(), sum_multiplier_.gpu_data(), float(0.),
          temp_.mutable_gpu_data());
