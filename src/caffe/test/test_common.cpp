...
TEST_F(CommonTest, TestRandSeedCPU) {
  ...
  caffe_rng_bernoulli(10, float(0.5), static_cast<int*>(data_a.mutable_cpu_data()));

  Caffe::set_random_seed(1701);
  caffe_rng_bernoulli(10, float(0.5), static_cast<int*>(data_b.mutable_cpu_data()));
...
#ifdef USE_CUDNN // GPU Caffe singleton test.
